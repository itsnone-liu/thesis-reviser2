...
function statusText(t){let m={queue:'等待生成',running:'正在生成',waiting_quota:'等待模型额度恢复',done:'已完成',error:'生成失败',interrupted:'已中断'};return m[t]||t}
function taskCard(t){let files=(t.files||[]).map(f=>'<a class="download" href="/download/'+encodeURIComponent(t.id)+'/'+encodeURIComponent(f)+'" target="_blank">下载 '+(f.endsWith('.docx')?'DOCX':'TXT')+'</a>').join(' ');let audit=t.audit||{};let warn=(audit.problems||[]).filter(x=>String(x).includes('⚠️')).join('；');return '<article class="task"><div class="task-head"><b>'+esc(t.paper_type)+'论文</b><span class="badge '+esc(t.status)+'">'+statusText(t.status)+'</span></div><p class="task-title">'+esc(t.title||'未填写题目')+'</p><div class="progress"><i style="width:'+Math.max(0,Math.min(100,t.progress||0))+'%"></i></div><p class="task-msg">'+esc(t.message||'等待开始')+'（'+(t.progress||0)+'%）</p>'+(t.error?'<p class="err">原因：'+esc(t.error)+'</p>':'')+(warn?'<p class="warn">'+esc(warn)+'</p>':'')+(files?'<div class="downloads">'+files+'</div>':'')+'<small>创建时间：'+esc(t.created_at||'')+'</small></article>'}
async function home(){try{let m=await api('/api/me'),d=await api('/api/tasks');$('app').innerHTML='<h1>个人论文工作台</h1><p>当前用户：'+esc(m.username)+'　角色：'+esc(m.role)+'</p><div class="grid"><button class="major" onclick="simple(\'管理\')">📊<strong>经管论文</strong></button><button class="major" onclick="simple(\'设计\')">🎨<strong>设计论文</strong></button><button class="major" onclick="simple(\'机械\')">⚙️<strong>机械论文</strong></button><button class="major" onclick="simple(\'土木\')">🏗️<strong>土木论文</strong></button><a class="major" href="/law">⚖️<strong>法学论文</strong></a></div><section><h3>我的任务</h3><p class="muted">历史任务会保留；生成完成后可下载 TXT 和 DOCX。</p><div id="tasks">'+(d.tasks.length?d.tasks.map(taskCard).join(''):'<p class="muted">暂无任务</p>')+'</div></section>'}catch(e){login()}}
